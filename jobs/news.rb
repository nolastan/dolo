...
news_feeds = {
  "reddit-sf" => "http://www.reddit.com/r/sanfrancisco.rss",
}
...
class News
  ...
  def latest_headlines
    ...
    news_headlines = [];
    atom_2_items = doc.xpath('//channel/item').to_a
    atom_2_items.each do |news_item|
    ...
    atom_1_items = doc.css('feed entry')
    atom_1_items.each do |news_item|
      title = clean_html( news_item.css('title').text )
      summary = clean_html( news_item.css('summary').text )
      news_headlines.push({ title: title, description: summary })
    end
    news_headlines
  end
